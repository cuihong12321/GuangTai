...
<script type="text/javascript">
    ...
    function addCommdity() {
        ...
        $('#submit').attr("onclick","checkCommdity()");
        $('#dlg').dialog('open').dialog('center').dialog('setTitle', '新增货物类型');

    }

    function checkCommdity() {
    ...
    function editCommdity() {
        ...
        $('#submit').attr("onclick","editcheckCommdity()");
        var row = $("#table").datagrid('getSelected');
        id = row.id;
        if(row!=null){
            $('#dlg').dialog('open').dialog('center').dialog('setTitle', '修改货物类型');
            $("#fm").form("load",row);
        }
        else {
            alert("请选择货物类型!");
    ...
    function deleteCommdity() {
        ...
        if(row!=null)
        {
            ...
            formData.append("id",id);
            $.ajax({
                url:"<%=contextPath%>/Commdity/Delete",
                type: 'POST',
                data: formData,
                cache: false,
                contentType: false,
...
<table id="table" class="easyui-datagrid" style="width:100%;height:auto"
       data-options="rownumbers:true,singleSelect:true,pagination:true,pageSize:20,url:'<%=contextPath%>/Commdity/Get',method:'post'">
    <thead>
    <tr>
        <th field="id" width=auto>ID</th>
        <th field="name" width=auto>货物名称</th>
...
<div id="toolbar">
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="addCommdity()">新增货物类型
    </a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editCommdity()">修改货物类型
    </a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="deleteCommdity()">移除货物类型</a>
</div>
<div id="dlg" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px"
     closed="true" buttons="#dlg-buttons">
    <div class="ftitle">货物类型</div>
