...
<script type="text/javascript">
    ...
    function addContainerType() {
        ...
        $('#submit').attr("onclick","checkContainerType()");
        $('#dlg').dialog('open').dialog('center').dialog('setTitle', '新增箱形');
    ...
    function checkContainerType() {
        ...
        if (name && explain) {
            $.ajax({
                url: "<%=contextPath%>/ContainerType/Add",
                type: 'POST',
                data: formData,
                cache: false,
                contentType: false,
    ...
    function editContainerType() {
        ...
        $('#submit').attr("onclick","editcheckContainerType()");
        var row = $("#table").datagrid('getSelected');
        id = row.id;
        if(row!=null) {
            $('#dlg').dialog('open').dialog('center').dialog('setTitle', '修改箱形');
            $("#fm").form("load", row);
        }
        else
        {
    ...
    function editcheckContainerType() {
        var formData = new FormData();
        var name = $.trim($("input[name='name']")[0].value);
        var explain = $.trim($("input[name='explain']")[0].value);
        ...
        {
            $.ajax({
                url:"<%=contextPath%>/ContainerType/Edit",
                type: 'POST',
                data: formData,
                cache: false,
                contentType: false,
    ...
    function deleteContainerType() {
        ...
        if(row!=null)
        {
            ...
            $.ajax({
                url:"<%=contextPath%>/ContainerType/Delete",
                type: 'POST',
                data: formData,
                cache: false,
                contentType: false,
...
<table id="table" class="easyui-datagrid" style="width:100%;height:auto"
       data-options="rownumbers:true,singleSelect:true,pagination:true,pageSize:20,url:'<%=contextPath%>/ContainerType/Get',method:'post'">
    <thead>
    <tr>
        <th field="id" width=auto>ID</th>
        <th field="name" width=auto>箱形</th>
...
<div id="toolbar">
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="addContainerType()">新增箱形
    </a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editContainerType()">修改箱形
    </a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true"
       onclick="deleteContainerType()">移除箱形</a>
</div>
<div id="dlg" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px"
     closed="true" buttons="#dlg-buttons">
    <div class="ftitle">箱形</div>
