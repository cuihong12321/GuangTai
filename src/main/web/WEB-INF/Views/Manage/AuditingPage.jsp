    ...
    $(function () {
        ...
        $('#table').datagrid({
            ...
            onExpandRow: function (index, row) {
                ...
                $('#ddv'+index).edatagrid({
                    url: '<%=contextPath%>/Cost/Get?ordercosttype=0&&orderid=' + row.id,
                    saveUrl:'<%=contextPath%>/Cost/Adds?ordercosttype=0&&orderid=' + row.id+'&&operatorid='+userid,
                    updateUrl: '<%=contextPath%>/Cost/Edit?ordercosttypeid=0&&orderid=' + row.id+'&&operatorid='+userid,
                    ...
                    columns: [[         //combobox的field被我改成xxxids,xxxid和下面弹出框的id冲突，
...
/*                        {field:'operatorid',title:'操作员',width:100,
                            formatter:function(value){                      //格式化该字段内的显示
                                for(var i=0; i<operatordata.length; i++){
                                    if(operatordata[i].id==value){
                                        operatorid=operatordata[i].id;
                                        return operatordata[i].name;
                                    }
                                }
                                return "nulls";
                            },
                ...
                $('#sddv' + index).edatagrid({
                    ...
                    rowStyler: function (indexson, rowson) {
                        if (rowson.ispayed == 0) {
                            return 'background-color:#D3D3D3;';
                        }
                        if (rowson.ispayed == 1) {
                    ...
                    toolbar: [{ text: '添加', iconCls: 'icon-add', handler: function () {
                        ...
                        }, '-',
                        {
                            ...
                            var selectedrow = $('#sddv' + index).datagrid('getSelected');
                                if(selectedrow.ispayed==0) {

                                    auditingRow(selectedrow, index, 1);
                                }
                                else
                                {
                                    alert("此记录不可审核");
                        ...
                        {
                            text: '撤销审核', handler: function () {
                            var selectedrow = $('#sddv' + index).datagrid('getSelected')
                                if(selectedrow.ispayed==1) {

                                    auditingRow(selectedrow, index, 0);
                                }
                                else
                                {
                                    alert("此记录不可撤销")
...
//                                    required:true
                        ...
                        {field: 'explain', title: '备注', width: '100', align: 'right',editor:'textbox'},
                        {field: 'id', title: 'ID', width: '100', align: 'right', hidden: 'true'},
                        {field: 'ispayed', title: '是否已付', width: '100', align: 'right'},
                    ]],
                    onResize: function () {
                        $('#table').datagrid('fixDetailRowHeight', index);
                    },
    ...
    function auditingRow(row,index,ispayed) {
        ...
        if(row.id!=null)
        {
            ...
            $.ajax({
                url: '<%=contextPath%>/Cost/AuditingCost',
                type: 'POST',
                dataType: 'json',
                cache: false,
                contentType: false,
    ...
    function completeWaybill() {
        ...
        if (row) {
            ...
            var factoryid = row.factoryid;
            var containerid = row.containerid;
            var billnumber = row.billnumber;
            var customerworknumber = row.customerworknumber;
            ...
            var operatetime = row.operatetime;
            var makingtime = row.makingtime;
            formData.append("id", row.id);
            formData.append("customerid", customerid);
            formData.append("factoryid", factoryid);
            formData.append("containerid", containerid);
            formData.append("billnumber", billnumber);
            formData.append("customerworknumber", customerworknumber);
            formData.append("orderworknumber", orderworknumber);
            formData.append("cabinetnumber", cabinetnumber);
            formData.append("sealnumber", sealnumber);
            formData.append("explain", explain);
            ...
            formData.append("operatetime", operatetime);
            formData.append("makingtime", makingtime);
            $.ajax({
...
/*    function editCostCostincom(index, indexson) {

        indexsonoveral = indexson;
        indexoveral = index;
        $('#table').datagrid('selectRow', index);
        var row = $('#ddv' + index).datagrid('getRows')[indexson];

        if (row) {
            $('#costsubmit').attr("onclick", "checkedityCostincom()");
    ...
    function checkedityCostincom() {
        var row = $("#table").datagrid('getSelected');
        var rowson = $('#ddv' + indexoveral).datagrid('getRows')[indexsonoveral];
        var formData = new FormData();
        var urlin = '<%=contextPath%>/Cost/Get?ordercosttype=0&&orderid=' + row.id;
        if (row && rowson) {
            var id = rowson.id;
            var orderid = row.id;
            ...
            formData.append("ordercosttype", ordercosttype);
            formData.append("costtype", costtypeid);
            formData.append("balanceunit", balanceunitid);
            formData.append("measure", measure);
            formData.append("currency", currencyid);
            formData.append("explain", explain);
            formData.append("price", price);
            formData.append("operator", row.operatorid);
    ...
    function checkedityCostpay() {
        ...
        if (row && rowson) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    alert(data);
                    $('#ddv' + indexoveral).datagrid("reload", urlin);
                },error:function (data) {
                    alert(data);
                    $('#ddv' + indexoveral).datagrid("reload", urlin);
                }
...
/*    function deleteCostpay(index, indexson) {
    ...
    function exportexcelreceive() {
        ...
        var    factoryid=$('#factoryid').combobox('getValue');
         var   containerid= $('#containerid').combobox('getValue');
         var   billnumber=$("#billnumber").val();
    var            customerworknumber= $("#customerworknumber").val();
  ...
  var          sealnumber= $("#sealnumber").val();
    ...
    var        begintime= $("#begintime").datebox("getValue");
       var     endtime= $("#endtime").datebox("getValue");
        ...
        window.location.href = "<%=contextPath%>/Order/WaybillStatistics/ExportExcel?customerid="+customerid +
            "&&factoryid="+factoryid+"&&containerid="+containerid+"&&billnumber="+billnumber+"&&customerworknumber="
        +customerworknumber+"&&cabinetnumber="+cabinetnumber+"&&sealnumber="+sealnumber+"&&operatorid="+operatorid+

        "&&state="+state+"&&begindate="+begindate+"&&enddate="+enddate+"&&begintime="+begintime+"&&endtime="+endtime+"&&type="+type;

    }
    function exportexceldeal() {
        var balanceunitid = $('#balanceunitid').combobox('getValue');
        var rows = $('#table').datagrid('getSelections');
        var type = 'deal';
        var ens = "";
        if(rows.length < 1){
        ...
        window.location.href = '<%=contextPath%>/Order/WaybillStatistics/ExportExcel?ens='+ens+'&&type='+type+"&&balanceunitid="+balanceunitid;
    }
...
<div id="aa" class="easyui-accordion">
    <div title="运单查询" iconcls="icon-search" style="overflow: auto; padding: 10px;">
        <div>
            ...
            <%--导出应付费用时的查询字段,仅在导出应付时可用--%>
            <label for="sealnumber">结算单位：</label>
            <input class="easyui-combobox" type="text" id="balanceunitid" name="balanceunit"
            ...
            <a href="javascript:exportexcelreceive();" class="easyui-linkbutton" data-options="iconCls:'icon-save'"
               style="width:100px">导出应收</a>
...
<table id="table" class="easyui-datagrid" fitColumns="false" style="width:100%;height:auto"
    ...
    <tr>
        ...
        <th field="sumincom" width=5%>应收总额</th>
