...
<script type="text/javascript">
    ...
    function addCostType() {
        ...
        $('#submit').attr("onclick","checkCostType()");
        $('#dlg').dialog('open').dialog('center').dialog('setTitle', '新增费用类型');

    }

    function checkCostType() {
    ...
    function editCostType() {
        ...
        $('#submit').attr("onclick","editcheckCostType()");
        var row = $("#table").datagrid('getSelected');
        id = row.id;
        if(row!=null){
        $('#dlg').dialog('open').dialog('center').dialog('setTitle', '修改费用类型');
        $("#fm").form("load",row);
    }
    else {
            alert("请选择费用类型!");
    ...
    function deleteCostType() {
        ...
        formData.append("id",id);
            ...
            $.ajax({
                url:"<%=contextPath%>/CostType/Delete",
                type: 'POST',
                data: formData,
                cache: false,
                contentType: false,
...
<table id="table" class="easyui-datagrid" style="width:100%;height:auto"
       data-options="rownumbers:true,singleSelect:true,pagination:true,pageSize:20,url:'<%=contextPath%>/CostType/Get',method:'post'">
    <thead>
    <tr>
        <th field="id" width=auto>ID</th>
        <th field="name" width=auto>费用名称</th>
...
<div id="toolbar">
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="addCostType()">新增费用类型
    </a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editCostType()">修改费用类型
    </a>
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="deleteCostType()">移除费用类型</a>
</div>
<div id="dlg" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px"
     closed="true" buttons="#dlg-buttons">
    <div class="ftitle">费用类型</div>
