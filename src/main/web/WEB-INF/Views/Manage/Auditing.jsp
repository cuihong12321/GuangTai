    ...
    $(function () {
        $('#table').datagrid({
            ...
            onExpandRow: function (index, row) {
                ...
                $('#ddv-' + index).datagrid({
                    url: '<%=contextPath%>/Cost/get?ordercosttype=0&&bulkorderid=' + row.id,
                    title: "应收",
                    fitColumns: false,
                    singleSelect: true,
                    rownumbers: true,
                    ...
                    height: 'auto',
                    columns: [[
                        {field: 'costtype', title: '费用类型', width: '100',align: 'right'},
                        ...
                        {field:'edit',width:50, title:"修改费用" ,width: '100', align:'center',formatter:function (value, row, indexson) {
                            ...
                            return '<a href="#" onclick="editCostCostincom(' + index + ','+ indexson+')">修改费用</a>';
                        }},
                        {field:'delete',width:50, title:"删除费用" ,width: '100', align:'center',formatter:  function (value, row, indexson) {
                            return '<a href="#" onclick="deleteCostincom(' + index + ','+ indexson+')">删除费用</a>';
                        }}
                    ]],
                    onResize: function () {
                    ...
                    onLoadSuccess: function () {
                        if(row.state==3)
                        {
                            $('#ddv-' + index).datagrid('hideColumn', 'edit');
                            $('#ddv-' + index).datagrid('hideColumn', 'delete');
                        }
                        setTimeout(function () {
                            $('#table').datagrid('fixDetailRowHeight', index);
                        }, 0);
...
//                return false;
    ...
    function editcheckOrder() {
        ...
        formData.append("pattern","auditing");
    ...
    function completeWaybill() {
        ...
        if(rows.length ==1 && rows[0].state == 2) {
            var formData = new FormData();
    ...
    function CheckWaybill() {
        ...
        if (ens) {
            ...
            formData.append("ens", ens);
            $.ajax({
                url: "<%=contextPath%>/BulkOrder/Check",
                type: 'POST',
                cache: false,
                contentType: false,
                processData: false,
    ...
    function checkWaybill() {
        var rows = $("#table").datagrid('getSelections');
        if(rows.length != 1 || rows[0].state != 3){
            alert("请选择单个已审核运单！");
            return ;
        }else{
            var formData = new FormData();
    ...
    function deleteCostincom(index,indexson) {
    indexsonoveral = indexson;
    indexoveral = index;
    var rowson = $('#ddv-' + index).datagrid('getRows')[indexson];
    deleteCost(rowson.id);
    }
    ...
    function exportexcel() {
        ...
        var commdityid = $('#commdityid').combobox('getValue');
        var customerid = $('#customerid').combobox('getValue');
        var startingid = $('#startingid').combobox('getValue');
        var destinationid = $('#destinationid').combobox('getValue');
        ...
        var beagindate = $('#begindate').combobox('getValue');
        var enddate = $('#enddate').combobox('getValue');
        var deliverytime = $('#delivery').combobox('getValue');
        var dispatchtime = $('#dispatch').combobox('getValue');

       /*window.location.href = '<%=contextPath%>/BulkOrder/WaybillStatistics/ExportExcel?commdityid='+commdityid +
                '&&customerid='+customerid+'&&startingid='+startingid+'&&destinationid='+destinationid+
                '&&operatorid='+operatorid+'&&state='+state+'&&customerworknumber='+customerworknumber+'&&begindate='
                +beagindate+'&&enddate='+enddate+'&&deliverytime='+deliverytime+'&&dispatchtime='+dispatchtime;*/
        if(state == 3) {
            window.location.href = '<%=contextPath%>/BulkOrder/WaybillStatistics/ExportExcel?commdityid='+commdityid +
                    '&&customerid='+customerid+'&&startingid='+startingid+'&&destinationid='+destinationid+
                    '&&operatorid='+operatorid+'&&state='+state+'&&customerworknumber='+customerworknumber+'&&begindate='
...
<div id="aa" class="easyui-accordion" style="width: auto">
    <div title="运单查询" iconcls="icon-search" style="overflow: auto; padding: 10px;">
        <div>
            ...
            <a href="javascript:exportexcel();" class="easyui-linkbutton" data-options="iconCls:'icon-save'"
                   style="width:100px">导出为Excel</a><br/><br/>
            </div>
        </div>
...
<table id="table" class="easyui-datagrid"  fitColumns="false" style="width:auto;height:auto"
    ...
    <tr>
        ...
        <th field="sumincom" width=10%>应收总额</th>
            ...
            <div class="fitem">
                ...
                <select  name="ordercosttype" id="ordercosttype" class="easyui-combobox" style="width:160px"
                         data-options="valueField:'id',textField:'name'" editable="false"
                           required="true">
                    <option value="0" id="0">应收</option>
                    <option value="1" id="1">应付</option>
                </select >
            </div>
            <div class="fitem">
                <label>费用类型</label>
